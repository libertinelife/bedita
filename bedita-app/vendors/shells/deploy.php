...
class DeployShell extends BeditaBaseShell {
    ...
    protected function printFolderList($list, $type = null, $startFrom = 1, $invert = false) {
        $number = $startFrom+1;
        foreach ($list as $key => $element) {
            $name = array_pop(explode(DS, $element));
            if ($invert && !empty($type)) {
                $str = ($number++) . '. ' . $type;
            } else {
                $str = ($number++) . '. ' . $name;
            }
            if (!empty($type) || $invert) {
                if ($invert) {
                    $str .= ' (' . $name . ')';
                } else {
                    $str .= ' (' . $type . ')';
    ...
    public function up() {
        ...
        $this->printFolderList($frontendFolders, 'frontend');
        $sel = array_merge($sel, $frontendFolders);

        $addonFolder = BeLib::getAddonFolders();
        $this->printFolderList($addonFolder, 'addon', count($sel), true);
        $sel = array_merge($sel, $addonFolder);

        $moduleFolder = BeLib::getPluginModuleFolders();
        $this->printFolderList($moduleFolder, 'plugin module', count($sel));
        $sel = array_merge($sel, $moduleFolder);
        ...
        $updateRes = BeLib::remoteUpdatePath($selected);

        if(!empty($updateRes['error'])) {
            $this->out($updateRes['error']);
        }

        $this->out('Update command: ' . $updateRes['command']);
        ...
        if($res == 1) {
            $this->updateVersion();
        } else if($res <= count($frontendFolders)+1) {
    ...
    public function changelog() {
        ...
        $currentBranch = BeLib::getGitBranch(ROOT);
        $branchesList = BeLib::getGitBranches();
        $this->out('BRANCHES:');
